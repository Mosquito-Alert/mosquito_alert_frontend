  ...
  <div class="flex-1 h-full overflow-hidden flex border border-surface rounded-2xl">
    ...
    >
        ...
        <template #body="slotProps">
          <RouterLink
            :to="{
              ...
              params: { observationUuid: slotProps.data.observation.uuid },
            }"
            custom
            v-slot="{ href }"
          >
      ...
      <Column header="Image">
        <template #body="slotProps">
          <Image
            :src="`${slotProps.data.public_photo.url}`"
            preview
      ...
      <Column header="Taxon">
        <template #body="slotProps">
          <IdentificationTaskResultTag
            v-if="slotProps.data.result"
            :result="slotProps.data.result"
          />
        </template>
      </Column>
      <Column header="Country">
        <template #body="slotProps">
          <CountryTag
            v-if="slotProps.data.observation.location.country"
      ...
      <Column header="Assignations">
        <template #body="slotProps">
          <AvatarGroup>
            <UserAvatar
              v-for="(assignment, index) in slotProps.data.assignments"
              :key="index"
              :user="assignment.user"
              v-tooltip.top="
                assignment.annotation_id === null
                  ? `${assignment.user.full_name || assignment.user.username} (Pending)`
                  : assignment.user.full_name || assignment.user.username
              "
              :class="
                assignment.annotation_id
                  ? ''
                  : 'border-(--p-tag-warn-color)! bg-(--p-tag-warn-background)! text-(--p-tag-warn-color)!'
              "
            />
          </AvatarGroup>
        </template>
      ...
      <Column field="is_flagged" header="Is flagged" dataType="boolean" style="min-width: 6rem">
        <template #body="slotProps">
          <i
            class="pi"
            :class="{
              'pi-check-circle text-green-500': slotProps.data.is_flagged,
              'pi-times-circle text-red-400': !slotProps.data.is_flagged,
            }"
          />
        </template>
      </Column>
