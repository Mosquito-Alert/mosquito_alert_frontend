<template>
  <figure class="relative">
    <router-link
      :to="{ name: 'identification_task', params: { observationUuid: task.observation.uuid } }"
      target="_blank"
      class="relative"
    >
      <div
    ...
    <figcaption
      v-if="task.observation.location.country"
      class="absolute top-2 right-2 rounded-md gap-1 flex flex-col"
    >
      <CountryTag
        v-if="task.observation.location.country"
        :country="task.observation.location.country"
    ...
    <figcaption
      v-else-if="$can('add', subject('Review', { identification_task: task }))"
  ...
  identificationTasksApi
    ...
    .then(() => {
      toast.add({
        severity: 'success',
        summary: 'Success',
        detail: `Annotated as '${taxon ? taxon.name : 'Not an insect'}'`,
        life: 3000,
      })
