<template>
  <Tag
    :severity="classification?.taxon && classification.is_high_confidence ? 'success' : 'secondary'"
    class="bg-transparent! border border-gray-500 relative"
  >
    <div class="flex items-center gap-2 px-1">
      <i
        v-if="classification?.taxon && classification.is_high_confidence"
      ...
      <span class="text-base text-surface-900 dark:text-surface-0">
        ...
        <i
          v-if="sex"
          :class="sex === SpeciesCharacteristicsSex.Male ? 'pi pi-mars' : 'pi pi-venus'"
