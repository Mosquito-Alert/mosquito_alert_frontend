  ...
  >
    <div class="flex flex-col overflow-auto gap-3 h-full">
      ...
      <div class="flex flex-row">
        ...
        <FormField
          v-if="canSetIsExecutive"
          v-slot="$field"
          name="isExecutive"
          :initial-value="false"
          class="flex ml-auto items-center gap-2"
        >
      ...
      <FormField v-slot="$field" name="internalNote" class="flex flex-col gap-2 w-full">
        <label
          >Internal note
          <span
            v-if="!isInternalNoteRequired"
            class="text-surface-500 dark:text-surface-300 italic"
          >
            (Optional)
          </span>
        </label>
      ...
      <FormField v-slot="$field" name="tags" :initial-value="[]" class="flex flex-col gap-2 w-full">
        <label
          >Tags <span class="text-surface-500 dark:text-surface-300 italic">(Optional)</span></label
        >
        <AutoComplete
    ...
    <div class="flex gap-2 w-full self-end">
      <Button
        ...
        iconPos="right"
        variant="outlined"
        @click="continueAfterSubmit = false"
      ...
      <Button
        ...
        icon="pi pi-upload"
        iconPos="right"
        @click="continueAfterSubmit = true"
        :loading="isSubmitting"
      />
    </div>
  </Form>
...
const props = withDefaults(
  ...
  }>(),
  {
    ...
    isFlagged: false,
    isFavourite: false,
    canSetIsExecutive: false,
  },
)

watch(selectedSex, (newSex) => {
...
const isInternalNoteRequired = computed(() => !props.isVisible || props.isFlagged)
...
const onFormSubmit = ({ valid, values }: { valid: boolean; values: Record<string, any> }) => {
  if (valid) {
    ...
    const annotationRequest = <AnnotationRequest>{
      ...
      characteristics:
        isExtended.value && selectedSex.value
          ? <SpeciesCharacteristicsRequest>{
...
const generatePublicNote = () => {
  if (selectedTaxon.value) {
    publicNote.value = getPublicNote(
      selectedTaxon.value,
      isHighConfidence.value,
      props.observation.user.locale || 'en',
    )
  }
}
</script>
