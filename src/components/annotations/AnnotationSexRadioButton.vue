  ...
  <div class="flex justify-center gap-4">
    <div v-for="(option, index) in options" :key="index">
      <RadioButton
        ...
        name="species-characteristics-sex"
        @update:modelValue="emit('update:modelValue', $event)"
        :inputId="`species-characteristics-sex-${index}`"
        :value="option.value"
        class="hidden"
        unstyled
      />
      <label
        :for="`species-characteristics-sex-${index}`"
...
defineProps<{
  modelValue?: SpeciesCharacteristicsSex | null
}>()
