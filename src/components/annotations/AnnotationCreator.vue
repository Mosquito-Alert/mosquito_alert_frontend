  ...
  <div v-bind="$attrs">
    <div class="flex size-full rounded-xl! bg-white">
      <Galleria
        ...
        :value="assignment.observation.photos"
        :numVisible="numVisible"
        thumbnailsPosition="right"
        :showThumbnailNavigators="assignment.observation.photos.length > numVisible"
        :containerClass="{
          'w-full': true,
          'h-full': true,
          'border-0!': true,
        ...
        circular
        :showThumbnails="!isStarted"
        :showItemNavigators="!isStarted && assignment.observation.photos.length > 1"
        :pt="{
      ...
      >
        ...
        <template #thumbnail="slotProps">
          <img
            v-if="!loading"
            :src="slotProps.item.url"
            class="max-h-full aspect-square object-scale-down"
          />
        </template>
        <template #caption>
          <div class="flex">
            ...
            >
              ...
              <Button
                ...
                class="not-hover:text-gray-200!"
                rounded
                variant="text"
                severity="secondary"
                roundedaria-label="Info"
                size="small"
                @click="observationDialogVisible = true"
              />
            </div>
            <div class="flex flex-col items-center p-4 gap-2 ml-auto">
              ...
              <div v-if="isStarted" class="flex flex-row ml-auto">
                <Button
                  v-if="assignment.observation.photos && assignment.observation.photos.length > 1"
                  label="Show all"
      ...
      <div
        class="flex flex-col w-200 h-full p-4 transition-all duration-500 ease-in-out"
      ...
      >
        <div v-if="isStarted" class="flex flex-col h-full">
          <div class="flex flex-row gap-2 items-center mb-4">
            ...
            <div class="flex flex-row ml-auto">
              ...
              <Button
                ...
                :severity="isFlagged ? 'danger' : 'secondary'"
                variant="text"
                rounded
                @click="
                  isFlagged = !isFlagged
          ...
          <AnnotationForm
            ...
            :annotation-type="assignment.annotation_type"
            :isVisible="isVisible"
            :isFlagged="isFlagged"
            :isFavourite="isFavourite"
            :can-set-is-executive="capabilities?.annotate_executive"
    ...
    <AnnotationGalleriaFullScreen
      :photos="assignment.observation.photos"
      v-model:visible="showGalleriaFullscreen"
      :best-photo="activePhoto"
    />
  </div>
