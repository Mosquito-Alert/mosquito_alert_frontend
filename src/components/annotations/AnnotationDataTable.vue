  ...
  <div class="flex-1 h-full overflow-hidden flex border border-surface rounded-2xl">
    ...
    >
          ...
          <Button asChild variant="link">
            <RouterLink
              :to="{
                ...
                params: { observationUuid: slotProps.data.observation_uuid },
              }"
              custom
              v-slot="{ href }"
            >
      ...
      <Column header="Best image">
        <template #body="slotProps">
          <Image
            v-if="slotProps.data.best_photo"
            :src="`${slotProps.data.best_photo.url}`"
            preview
      ...
      <Column header="Classification">
        <template #body="slotProps">
          <TaxonClassificationTag
            :classification="slotProps.data.classification"
      ...
      <Column field="is_favourite" header="Is Favourite" dataType="boolean" style="min-width: 6rem">
        <template #body="slotProps">
          <i
            class="pi"
            :class="{
              'pi-check-circle text-green-500': slotProps.data.observation_flags.is_favourite,
              'pi-times-circle text-red-400': !slotProps.data.observation_flags.is_favourite,
            }"
          />
        </template>
      </Column>
      ...
      <template #expansion="slotProps">
        ...
        <p>
          <span class="font-bold">Internal note: </span>{{ slotProps.data.feedback.internal_note }}
        </p>
      </template>
    </DataTable>
  </div>
