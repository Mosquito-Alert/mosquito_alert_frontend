<template>
  <div class="flex flex-col gap-4 w-full p-2">
    <div class="flex pt-2 gap-2">
      <div class="flex flex-row w-full items-center gap-2">
        <InputGroup>
          <InputGroupAddon>
            <Button
              :icon="formTypeOptions.find((option) => option.value === selectedFormType)?.icon"
              severity="secondary"
              variant="text"
              @click="toggleFormTypeMenu"
              :disabled="disableRecipientSelect"
            />
          </InputGroupAddon>
          <FloatLabel variant="on">
            <UserAutocomplete
              v-if="selectedFormType === 'user'"
              v-model="userRecipients"
              :disabled="disableRecipientSelect"
              multiple
              class="[&>*]:rounded-l-none!"
            />
            <MessageTopicAutocomplete
              v-else
            ...
            <template #value="slotProps">
              <div v-if="slotProps.value" class="flex gap-2 items-center">
                ...
                <div v-if="languageIsRequired(slotProps.value)">
                  <i
                    v-if="!isLanguageComplete(slotProps.value)"
            ...
            <template #option="slotProps">
              <div class="flex gap-2 items-center">
                ...
                <div v-if="languageIsRequired(slotProps.option)">
                  <i
                    v-if="!isLanguageComplete(slotProps.option)"
                    class="pi pi-times-circle text-red-500"
                  />
                  <i v-else class="pi pi-check-circle text-green-500" />
                </div>
    ...
    <div v-if="hasRecipient" class="flex flex-col gap-4 mt-4">
      <div class="flex gap-1">
        <FloatLabel variant="on">
          <InputText
            ...
            v-model="subjectByLanguage[selectedLanguage]"
            type="text"
            fluid
            :lang="selectedLanguage"
            :invalid="!isSubjectComplete(selectedLanguage)"
          />
      ...
      <Editor
        v-if="selectedLanguage"
        v-model="bodyByLanguage[selectedLanguage]"
        :invalid="!isBodyComplete(selectedLanguage)"
...
const TOPIC_LANGUAGES = [
  ...
  'tr',
  'zh-cn',
] as const satisfies ReadonlyArray<keyof LocalizedTopicMessageTitleRequest>
...
const bodyByLanguage = ref<
  Record<
    keyof LocalizedMessageBodyRequest | keyof LocalizedTopicMessageBodyRequest,
    string | undefined
  >
>({} as Record<keyof LocalizedMessageBodyRequest, string | undefined>)
const subjectByLanguage = ref<
  Record<
    keyof LocalizedMessageTitleRequest | keyof LocalizedTopicMessageTitleRequest,
    string | undefined
  >
>(
  {} as Record<
...
watch(selectedFormType, () => {
  ...
  subjectByLanguage.value = {} as Record<
    keyof LocalizedMessageTitleRequest | keyof LocalizedTopicMessageTitleRequest,
    string | undefined
  >
...
const languageIsRequired = (
  lang: keyof LocalizedMessageBodyRequest | keyof LocalizedMessageTitleRequest | string,
) => {
  return requiredLanguages.value.includes(
    lang as keyof LocalizedMessageBodyRequest | keyof LocalizedMessageTitleRequest,
  )
}

const requiredLanguages = computed(() => {
  const languages = new Set<
    keyof LocalizedMessageBodyRequest | keyof LocalizedMessageTitleRequest
  >()
      ...
      if (recipient.locale) {
        languages.add(
          recipient.locale as
            ...
            | keyof LocalizedMessageTitleRequest,
        )
      }
    })
...
const selectedLanguage = ref<
  keyof LocalizedMessageBodyRequest | keyof LocalizedMessageTitleRequest | null
>(null)
...
watch(requiredLanguages, (newLanguages, oldLanguages) => {
  ...
  if (!selectedLanguage.value && newLanguages.length > 0) {
    selectedLanguage.value = newLanguages[0] as
      | keyof LocalizedMessageBodyRequest
      | keyof LocalizedMessageTitleRequest
  }
  ...
  if (
    ...
    !newLanguages.includes(selectedLanguage.value) &&
...
const isSubjectComplete = (
  lang: keyof LocalizedMessageBodyRequest | keyof LocalizedMessageTitleRequest,
) => {
