<template>
  <div class="card">
    ...
    <div class="flex flex-col py-4 gap-4">
      <div class="flex flex-row">
        <div class="flex gap-2">
          <IconField>
            ...
            <InputText
              placeholder="Search by uuid or id"
      ...
      >
        <FloatLabel variant="on">
          <IdentificationTaskStatusMultiSelect
            id="status_filter"
        ...
        <FloatLabel variant="on">
          <Select
            id="num_annotations"
            v-model="selectedNumAnnotation"
            class="w-35"
            :options="numAnnotationOptions"
            optionValue="value"
            optionLabel="label"
            dataKey="value"
        ...
        <FloatLabel variant="on">
          <IdentificationTaskReviewActionSelect
            id="review_action"
        ...
        <FloatLabel variant="on">
          <DatePicker
            ...
            v-model="selectedCreatedAtDateRange"
            showIcon
            iconDisplay="input"
            :max-date="new Date()"
            selectionMode="range"
        ...
        <FloatLabel variant="on">
          <DatePicker
            id="updated_at_filter"
            v-model="selectedUpdatedAtDateRange"
            showIcon
            iconDisplay="input"
            :max-date="new Date()"
            selectionMode="range"
    ...
    <DataView
      :value="identificationTasksArray"
      dataKey="observation.uuid"
      v-model:rows="numRows"
      :total-records="identificationTasksTotalCount"
      :layout="layout"
      lazy
      paginator
...
const numAnnotationOptions = ref<
  Array<{ value: { min: number | undefined; max: number | undefined }; label: string }>
>([
...
// Order by
const selectedOrderBy = ref<{ value: IdentificationtasksListOrderByParameter; label: string }>({
...
onMounted(() => {
  ...
  selectedSex.value = q.sex
    ? (q.sex as IdentificationtasksListResultCharacteristicsSexParameter)
    : undefined
...
const listRequest = computed<IdentificationTasksApiListRequest>(() => ({
  createdAtAfter:
    selectedCreatedAtDateRange.value && selectedCreatedAtDateRange.value.length > 1
      ? selectedCreatedAtDateRange.value[0].toISOString()
      : undefined,
  createdAtBefore:
    selectedCreatedAtDateRange.value && selectedCreatedAtDateRange.value.length > 1
      ? new Date(
          new Date(selectedCreatedAtDateRange.value[1]).setDate(
            selectedCreatedAtDateRange.value[1].getDate() + 1,
          ),
        ).toISOString()
      : undefined,
  updatedAtAfter:
    selectedUpdatedAtDateRange.value && selectedUpdatedAtDateRange.value.length > 1
      ? selectedUpdatedAtDateRange.value[0].toISOString()
      : undefined,
  updatedAtBefore:
    selectedUpdatedAtDateRange.value && selectedUpdatedAtDateRange.value.length > 1
      ? new Date(
          new Date(selectedUpdatedAtDateRange.value[1]).setDate(
            selectedUpdatedAtDateRange.value[1].getDate() + 1,
          ),
        ).toISOString()
      : undefined,
...
const fetchData = async () => {
  loading.value = true
  await identificationTasksApi
    .list(listRequest.value)
    .then((response) => {
