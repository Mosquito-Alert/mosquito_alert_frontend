<template>
  <div class="flex flex-col flex-1 h-[calc(100dvh-8rem)]">
    <AnnotationCreator
      v-if="assignment"
      class="flex-1 h-px w-full"
      :assignment="assignment"
      :capabilities="identificationTaskCapabilities"
      :loading="loading"
      @on-start-annotation="annotationStarted = true"
      @on-cancel-annotation="annotationStarted = false"
...
onMounted(async () => {
  ...
  if (a) {
    ...
    toast.add({
      severity: 'error',
      summary: 'No assignment found',
      detail: 'This identification task has not been assigned to you.',
      life: 3000,
    })
  }
})
...
watch(assignment, async (newAssignment) => {
  ...
  if (newAssignment?.observation.uuid) {
    try {
      const response = await identificationTasksApi.capabilitiesRetrieve({
  ...
  if (
    newAssignment?.observation.uuid &&
    newAssignment.observation.uuid !== route.params.observationUuid
  ) {
    router.push({
...
const fetchNewAssignment = async () => {
  ...
  if (nextAssignment) {
    ...
    toast.add({
      severity: 'info',
      summary: 'No more assignments',
      detail: 'You have completed all available assignments.',
      life: 3000,
    })
