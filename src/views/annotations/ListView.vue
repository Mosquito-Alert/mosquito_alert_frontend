<template>
  <div class="card">
    ...
    <DataView
      :value="annotationsArray"
      dataKey="id"
      v-model:rows="numRows"
      :total-records="annotationsTotalCount"
      lazy
      paginator
...
import type {
  Annotation,
  IdentificationTasksApiAnnotationsListMineRequest,
  Taxon,
} from 'mosquito-alert'
...
onMounted(() => {
  ...
  selectedSex.value = q.sex
    ? (q.sex as IdentificationtasksAnnotationsListCharacteristicsSexParameter)
    : undefined
...
const listRequest = computed<IdentificationTasksApiAnnotationsListMineRequest>(
  () =>
    ({
      updatedAtAfter:
        selectedDateRange.value && selectedDateRange.value.length > 1
          ? selectedDateRange.value[0].toISOString()
          : undefined,
      updatedAtBefore:
        selectedDateRange.value && selectedDateRange.value.length > 1
          ? new Date(
              new Date(selectedDateRange.value[1]).setDate(
                selectedDateRange.value[1].getDate() + 1,
              ),
            ).toISOString()
          : undefined,
...
const fetchData = async () => {
  ...
  await identificationTasksApi
    .annotationsListMine(listRequest.value)
    .then((response) => {
